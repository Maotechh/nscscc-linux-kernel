...
Sub Main
    ...
    If ok Then ok = RunLinuxCommand(statusFile, "cat /etc/nscscc/desktop-build-info", "__DONE_DESKTOP_BUILD_INFO__", 30, "ba583e6a067a16316aaaff1d6a084a56b548d359")
    ...
    If ok Then ok = RunLinuxCommand(statusFile, "dmesg | grep -E 'ue11-hcd|new full-speed USB device|New USB device found|hid-generic'", "__DONE_USB_PROBE__", 60, "ue11-hcd")
    If ok Then ok = RunLinuxCommand(statusFile, "lsusb", "__DONE_USB_LSUSB__", 30, "Bus 001")
    If ok Then ok = RunLinuxCommand(statusFile, "grep -A8 -B2 -i 'mouse' /proc/bus/input/devices", "__DONE_USB_INPUT__", 30, "Handlers=")
    If ok Then ok = RunLinuxCommand(statusFile, "ls /dev/hidraw* >/dev/null 2>&1 && echo HIDRAW_PRESENT", "__DONE_HIDRAW__", 30, "HIDRAW_PRESENT")
    If ok Then ok = RunLinuxCommand(statusFile, "MOUSE_EVENT=$(awk '/Handlers=.*mouse/ {for (i=1;i<=NF;i++) if ($i ~ /^event/) {print $i; exit}}' /proc/bus/input/devices); test -n ""$MOUSE_EVENT"" && echo MOUSE_EVENT=$MOUSE_EVENT", "__DONE_USB_MOUSE_EVENT__", 30, "MOUSE_EVENT=event")
    If ok Then ok = RunLinuxCommand(statusFile, "MOUSE_EVENT=$(awk '/Handlers=.*mouse/ {for (i=1;i<=NF;i++) if ($i ~ /^event/) {print $i; exit}}' /proc/bus/input/devices); timeout 45 evtest /dev/input/$MOUSE_EVENT > /tmp/nscscc-mouse-evtest.log 2>&1; rc=$?; test $rc = 124 || test $rc = 0", "__DONE_USB_EVTEST_CAPTURE__", 60, "")
    If ok Then ok = RunLinuxCommand(statusFile, "grep -E 'type 2 \(EV_REL\), code (0|1) \(REL_(X|Y)\)' /tmp/nscscc-mouse-evtest.log", "__DONE_USB_REL_EVENTS__", 30, "EV_REL")
    If ok Then ok = RunLinuxCommand(statusFile, "grep -E 'type 1 \(EV_KEY\), code 272 \(BTN_LEFT\)' /tmp/nscscc-mouse-evtest.log", "__DONE_USB_BUTTON_EVENT__", 30, "BTN_LEFT")
    ...
    If ok Then ok = RunLinuxCommand(statusFile, "cat /sys/class/graphics/fb0/virtual_size /sys/class/graphics/fb0/bits_per_pixel", "__DONE_FB0_MODE__", 30, "480,800")
    If ok Then ok = RunLinuxCommand(statusFile, "test ""$(cat /sys/class/graphics/fb0/bits_per_pixel)"" = 16 && echo FB0_RGB565_OK", "__DONE_FB0_RGB565__", 30, "FB0_RGB565_OK")
    If ok Then ok = RunLinuxCommand(statusFile, "test -s /run/nscscc-desktop.pid && kill -0 ""$(cat /run/nscscc-desktop.pid)"" && echo XINIT_RUNNING", "__DONE_XINIT__", 30, "XINIT_RUNNING")
    If ok Then ok = RunLinuxCommand(statusFile, "i=0; while [ ""$i"" -lt 120 ]; do pidof Xorg >/dev/null && pidof fluxbox >/dev/null && pidof xterm >/dev/null && break; i=$((i+1)); sleep 1; done; pidof Xorg >/dev/null && pidof fluxbox >/dev/null && pidof xterm >/dev/null && echo DESKTOP_READY", "__DONE_DESKTOP_READY__", 180, "DESKTOP_READY")
    If ok Then ok = RunLinuxCommand(statusFile, "MEM_AVAILABLE_KB=$(awk '/^MemAvailable:/ {print $2}' /proc/meminfo); test ""$MEM_AVAILABLE_KB"" -gt 8192 && echo DESKTOP_MEMORY_OK available_kb=$MEM_AVAILABLE_KB", "__DONE_DESKTOP_MEMORY__", 30, "DESKTOP_MEMORY_OK")
    ...
    If ok Then ok = RunLinuxCommand(statusFile, "grep -E 'Using input driver .evdev.|config/udev: Adding input device' /var/log/Xorg.0.log", "__DONE_XORG_EVDEV__", 30, "evdev")
