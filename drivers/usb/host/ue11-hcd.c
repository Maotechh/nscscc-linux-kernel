...
/* Added by the JIT-THU V0.5 register block. */
...
static void usbhw_hub_enable(struct ue11 *ue11, int full_speed, int enable_sof)
{
    ...
    writel(1 << USB_CTRL2_TX_FLUSH, ue11->reg_base + USB_CTRL2);
    writel(val, ue11->reg_base + USB_CTRL);
}

static void usbhw_phy_reset(struct ue11 *ue11)
{
    writel(1 << USB_CTRL2_PHY_RESET, ue11->reg_base + USB_CTRL2);
    udelay(10);
...
static void process_transfer_result(struct ue11 *ue11, struct ue11h_ep *ep)
{
    ...
    transaction_error = status &
        ((1 << USB_RX_STAT_START_PEND_SHIFT) |
         (1 << USB_RX_STAT_CRC_ERR_SHIFT) |
         (1 << USB_RX_STAT_RESP_TIMEOUT_SHIFT));
    if (transaction_error) {
        dev_dbg_ratelimited(ue11_to_hcd(ue11)->self.controller,
                            "transaction error: status=%08x pid=%02x\n",
                            status, ep->nextpid);
        response = 0;
    }
...
static int ue11h_probe(struct platform_device *dev)
{
    ...
    dev_info(&dev->dev,
             "V0.5 controller at %pa, IRQ %d; low-speed devices unsupported\n",
             &iores->start, irq);
...
static const struct of_device_id my_match_table[] = {
     { .compatible = "ultraembedded,usb-host-fs" },
     { .compatible = "ue11-hcd" },
     {},
};
